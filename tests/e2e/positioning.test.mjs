...
test.describe("Positioning Edge Cases", () => {
  ...
  test("window resize: picker repositions", async () => {
    ...
    const isBelow =
      pickerBoxAfterResize.y > inputBoxAfterResize.y + inputBoxAfterResize.height
    const isAbove = pickerBoxAfterResize.y + pickerBoxAfterResize.height < inputBoxAfterResize.y
    ...
    expect(pickerBoxAfterResize.x + pickerBoxAfterResize.width).toBeLessThanOrEqual(
      newViewportWidth + 10,
    ) // Allow small margin
